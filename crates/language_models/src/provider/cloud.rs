...
/// Jitter of up to the same duration is added on each scheduling to avoid synchronized
/// fetches from many clients after a cloud deployment.
...
impl State {
        ...
        let cloud_reconnect_task = cx.spawn({
            ...
            async move |this, cx| {
                ...
                while let Some(connection_id) = connection_id_rx.next().await {
                    ...
                    // since this `Client` was created; only real (re)connects trigger a
                    // refresh.
                    if connection_id == 0 {
                        continue;
                    }
    ...
    /// Schedules a model list refresh `MODELS_REFRESH_DEBOUNCE` (plus jitter) from now,
    /// replacing any previously scheduled refresh. Used to coalesce bursts of cloud
    /// websocket reconnects (e.g. during a rolling deploy) into a single refresh.
