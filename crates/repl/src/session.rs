            ...
            let close_button = h_flex()
                ...
                .child(
                    IconButton::new("close_output_area", IconName::Close)
                        ...
                        .tooltip(Tooltip::text(l10n::text("Close output area")))
    ...
    fn render(&mut self, _: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        let (status_text, interrupt_button) = match &self.kernel {
            Kernel::RunningKernel(kernel) => (
                ...
                Some(
                    Button::new("interrupt", l10n::text("Interrupt"))
            ...
            Kernel::StartingKernel(_) => (Some(l10n::text("Starting").into()), None),
            Kernel::ErroredLaunch(err) => (Some(format!("{} {err}", l10n::text("Error:"))), None),
            Kernel::ShuttingDown => (Some(l10n::text("Shutting Down").into()), None),
            Kernel::Shutdown => (Some(l10n::text("Shutdown").into()), None),
            Kernel::Restarting => (Some(l10n::text("Restarting").into()), None),
        };
        ...
        KernelListItem::new(self.kernel_specification.clone())
            ...
            .button(
                Button::new("shutdown", l10n::text("Shutdown"))
                    .style(ButtonStyle::Subtle)
