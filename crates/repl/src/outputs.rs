...
use ui::{CommonAnimationExt, CopyButton, IconButton, Tooltip, l10n, prelude::*};
            ...
            h_flex()
                ...
                .when(v.has_clipboard_content(window, cx), |el| {
                    ...
                    el.child(
                        IconButton::new(ElementId::Name("copy-output".into()), IconName::Copy)
                            ...
                            .tooltip(Tooltip::text(l10n::text("Copy Output")))
                ...
                .when(v.has_buffer_content(window, cx), |el| {
                    ...
                    el.child(
                        ...
                        .style(ButtonStyle::Transparent)
                        .tooltip(Tooltip::text(l10n::text("Open in Buffer")))
                        .on_click({
                ...
                Self::ErrorOutput(err) => Some(
                    h_flex()
                        ...
                        .child(
                            ...
                            .style(ButtonStyle::Transparent)
                            .tooltip(Tooltip::text(l10n::text("Open Full Error in Buffer")))
                            .on_click({
                                let ename = err.ename.clone();
                                let evalue = err.evalue.clone();
                                let traceback = err.traceback.clone();
    ...
    pub fn handle_input_request(
        ...
        if let JupyterMessageContent::InputRequest(input_request) = &message.content {
            ...
            let editor = cx.new(|cx| {
                ...
                editor.set_placeholder_text(l10n::text("Type here and press Enter"), window, cx);
                if password {
    ...
    fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        let status = match &self.status {
            ExecutionStatus::ConnectingToKernel => {
                Label::new(l10n::text("Connecting to kernel..."))
                    .color(Color::Muted)
                    .into_any_element()
            }
            ExecutionStatus::Executing => h_flex()
                ...
                .child(Label::new(l10n::text("Executing...")).color(Color::Muted))
                .into_any_element(),
            ExecutionStatus::Finished => Icon::new(IconName::Check)
                .size(IconSize::Small)
                .into_any_element(),
            ExecutionStatus::Unknown => Label::new(l10n::text("Unknown status"))
                .color(Color::Muted)
                .into_any_element(),
            ExecutionStatus::ShuttingDown => Label::new(l10n::text("Kernel shutting down..."))
                .color(Color::Muted)
                .into_any_element(),
            ExecutionStatus::Restarting => Label::new(l10n::text("Kernel restarting..."))
                .color(Color::Muted)
                .into_any_element(),
            ExecutionStatus::Shutdown => Label::new(l10n::text("Kernel shutdown"))
                .color(Color::Muted)
                .into_any_element(),
            ExecutionStatus::Queued => Label::new(l10n::text("Queued..."))
                .color(Color::Muted)
                .into_any_element(),
