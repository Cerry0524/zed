...
use ui::{
    ...
    Indicator, PopoverMenu, PopoverMenuHandle, ProgressBar, Tooltip, l10n, prelude::*,
};
        ...
        match language_settings.edit_predictions.provider {
                                    ...
                                    workspace.update(cx, |workspace, cx| {
                                        ...
                                        workspace.show_toast(
                                            Toast::new(
                                                ...
                                                l10n::text("Copilot can't be started: {error}")
                                                    .replace("{error}", &e.to_string()),
                                            )
                                            .on_click(
                                                l10n::text("Reinstall Copilot"),
                                                move |window, cx| {
            ...
            EditPredictionProvider::Codestral => {
                ...
                let tooltip_meta = if has_api_key {
                    l10n::text("Powered by Codestral")
                } else {
                    l10n::text("Missing API key for Codestral")
                };
                            ...
                            move |_window, cx| {
                                ...
                                let tooltip_meta = match settings.edit_predictions.ollama.as_ref() {
                                    Some(settings) if !settings.model.trim().is_empty() => {
                                        l10n::text("Powered by Ollama ({model})")
                ...
                match provider {
                    EditPredictionProvider::Mercury => {
                        ...
                        tooltip_meta = if missing_token {
                            l10n::text("Missing API key for Mercury")
                        } else if mercury_has_error {
                            l10n::text("Mercury free tier limit reached")
                        } else {
                            l10n::text("Powered by Mercury")
                        };
                    }
                    _ => {
                        ep_icon = if enabled { icons.base } else { icons.disabled };
                        tooltip_meta = l10n::text("Powered by Zeta")
                    }
                };

                if edit_prediction::should_show_upsell_modal(cx) {
                    let tooltip_meta = if self.user_store.read(cx).current_user().is_some() {
                        l10n::text("Choose a Plan")
                    } else {
                        l10n::text("Configure a Provider")
                    };
                    ...
                    .when(!self.popover_menu_handle.is_deployed(), |element| {
                        element.tooltip(move |_window, cx| {
                            let description = if !enabled {
                                l10n::text("Disabled For This File")
                            } else if zed_cloud_needs_sign_in {
                                l10n::text("Sign In Or Configure a Provider")
                            } else if provider_unavailable || show_editor_predictions {
                                tooltip_meta
                            } else {
                                l10n::text("Enable to Use")
                            };
...
impl EditPredictionButton {
    ...
    fn add_provider_switching_section(
        ...
        if !providers.is_empty() {
            menu = menu.separator().header(l10n::text("Providers"));
    ...
    fn add_configure_providers_item(&self, menu: ContextMenu) -> ContextMenu {
        menu.separator().item(
            ContextMenuEntry::new(l10n::text("Configure Providers"))
    ...
    pub fn build_copilot_start_menu(
        ...
        ContextMenu::build(window, cx, |menu, _, cx| {
            let menu = menu
                .entry(l10n::text("Sign In to Copilot"), None, move |window, cx| {
                    telemetry::event!(
                        "Edit Prediction Menu Action",
                        action = "sign_in",
                        provider = "copilot",
                ...
                .entry(l10n::text("Disable Copilot"), None, {
                    let fs = fs.clone();
                    move |_window, cx| {
                        telemetry::event!(
                            "Edit Prediction Menu Action",
    ...
    pub fn build_language_settings_menu(
        ...
        menu = menu.header(l10n::text("Show Edit Predictions For"));
        ...
        if let Some(editor_focus_handle) = self.editor_focus_handle.clone() {
            let entry = ContextMenuEntry::new(l10n::text("This Buffer"))
                        ...
                        entry
                            ...
                            .documentation_aside(DocumentationSide::Left, move |_cx| {
                                Label::new(
                                    ...
                                    .replace("{language}", language.name().as_ref()),
                                )
                                    .into_any_element()
                            })
                    );
        ...
        let entry = ContextMenuEntry::new(l10n::text("All Files"))
        ...
        menu = menu
                .separator()
                .header(l10n::text("Display Modes"))
                .item(
                    ContextMenuEntry::new(l10n::text("Eager"))
                        .toggleable(IconPosition::Start, eager_mode)
                        .documentation_aside(DocumentationSide::Left, move |_| {
                            Label::new(l10n::text("Display predictions inline when there are no language server completions available.")).into_any_element()
                        })
                        .handler({
                            let fs = fs.clone();
                            move |_, cx| {
                ...
                .item(
                    ContextMenuEntry::new(l10n::text("Subtle"))
                        .toggleable(IconPosition::Start, subtle_mode)
                        .documentation_aside(DocumentationSide::Left, move |_| {
                            Label::new(l10n::text("Display predictions inline only when holding a modifier key (alt by default).")).into_any_element()
                        })
                        .handler({
                            let fs = fs.clone();
                            move |_, cx| {
        ...
        menu = menu.separator().header(l10n::text("Privacy"));
        ...
        if matches!(provider, EditPredictionProvider::Zed) {
            if let Some(provider) = &self.edit_prediction_provider {
                ...
                if data_collection.is_supported() {
                    ...
                    menu = menu.item(
                        ContextMenuEntry::new(l10n::text("Training Data Collection"))
                            ...
                            .documentation_aside(DocumentationSide::Left, move |cx| {
                                let (msg, label_color, icon_name, icon_color) = match (is_open_source, is_collecting) {
                                    (true, true) => (
                                        l10n::text("Project identified as open source, and you're sharing data."),
                                        Color::Default,
                                        IconName::Check,
                                        Color::Success,
                                    ),
                                    (true, false) => (
                                        l10n::text("Project identified as open source, but you're not sharing data."),
                                        Color::Muted,
                                        IconName::Close,
                                        Color::Muted,
                                    ),
                                    (false, true) => (
                                        l10n::text("Project not identified as open source. No data captured."),
                                        Color::Muted,
                                        IconName::Close,
                                        Color::Muted,
                                    ),
                                    (false, false) => (
                                        l10n::text("Project not identified as open source, and setting turned off."),
                                        Color::Muted,
                                        IconName::Close,
                                        Color::Muted,
                                    ),
                    ...
                    if is_collecting && !is_open_source {
                        menu = menu.item(
                            ContextMenuEntry::new(l10n::text("No data captured."))
                                .disabled(true)
                                .icon(IconName::Close)
        ...
        menu = menu.item(
            ContextMenuEntry::new(l10n::text("Configure Excluded Files"))
            ...
            ContextMenuEntry::new(l10n::text("View Docs"))
        ...
        if !self.editor_enabled.unwrap_or(true) {
            ...
            menu = menu.item(
                ContextMenuEntry::new(l10n::text("This file is excluded."))
                    .disabled(true)
                    .icon(icons.disabled)
                    .icon_size(IconSize::Small),
            );
        ...
        if let Some(editor_focus_handle) = self.editor_focus_handle.clone() {
            menu = menu
                .separator()
                .header(l10n::text("Actions"))
                .entry(
                    l10n::text("Predict Edit at Cursor"),
    ...
    fn build_copilot_context_menu(
        ...
        ContextMenu::build(window, cx, |menu, window, cx| {
            ...
            let menu = menu
                .separator()
                .item(
                    ContextMenuEntry::new(l10n::text("Copilot: Next Edit Suggestions"))
                ...
                .separator()
                .link(
                    l10n::text("Go to Copilot Settings"),
                    OpenBrowser { url: settings_url }.boxed_clone(),
                )
                .action(l10n::text("Sign Out"), copilot::SignOut.boxed_clone());
            menu
        })
    }
            ...
            if needs_sign_in {
                menu = menu
                    .custom_row(move |_window, cx| {
                        ...
                        v_flex()
                            ...
                            .child(Label::new(l10n::text("Edit Prediction")))
                            .child(
                    ...
                    .separator()
                    .entry(l10n::text("Sign In & Start Using"), None, |window, cx| {
                        telemetry::event!(
                            "Edit Prediction Menu Action",
                            action = "sign_in",
                            provider = "zed",
                ...
                if mercury_payment_required {
                    menu = menu
                        .header("Mercury")
                        .item(
                            ContextMenuEntry::new(l10n::text("Free tier limit reached"))
                                .disabled(true),
                        )
                        .item(
                            ContextMenuEntry::new(l10n::text(
                ...
                if let Some(usage) = self
                    ...
                    .and_then(|provider| provider.usage(cx))
                {
                    menu = menu.header(l10n::text("Usage"));
                    menu = menu
                        .custom_entry(
                            move |_window, cx| {
                        ...
                        .when(usage.over_limit(), |menu| -> ContextMenu {
                            menu.entry(
                                l10n::text("Subscribe to increase your limit"),
                                None,
                                |_window, cx| {
                                    telemetry::event!(
                                        "Edit Prediction Menu Action",
                    ...
                    menu = menu
                        ...
                        .entry(
                            l10n::text("Upgrade to Zed Pro or contact us."),
                            None,
                            |_window, cx| {
                                telemetry::event!(
                                    "Edit Prediction Menu Action",
                    ...
                    menu = menu
                        .custom_entry(
                            |_window, _cx| {
                                Label::new(l10n::text("You have an outstanding invoice"))
                                    .size(LabelSize::Small)
                                    .color(Color::Warning)
                                    .into_any_element()
                            },
                        ...
                        .entry(
                            l10n::text("Check your payment status or contact us at billing-support@zed.dev to continue using this feature."),
                            None,
                            |_window, cx| {
                                cx.open_url(&zed_urls::account_url(cx))
                            },
